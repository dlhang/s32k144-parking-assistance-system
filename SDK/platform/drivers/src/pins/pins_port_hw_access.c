...
static uint32_t PINS_GetAdcInterleaveVal(const PORT_Type *base,
		const uint32_t pinPortIdx, const uint32_t currentVal) {
	uint32_t adcInterleaveVal = (uint32_t) PIN_ADC_INTERLEAVE_INVALID;
	/* calculate appropriate value to enable or disable in SIM_CHIPCTL[ADC_INTERLEAVE_EN] */
	if ((uint32_t) base == (uint32_t) PORTB) {
		switch (pinPortIdx) {
		case 0:
			adcInterleaveVal = (uint32_t) PIN_ADC_INTERLEAVE_ENABLE0
					| currentVal;
			break;
		case 1:
			adcInterleaveVal = (uint32_t) PIN_ADC_INTERLEAVE_ENABLE1
					| currentVal;
			break;
		case 13:
			adcInterleaveVal = (uint32_t) PIN_ADC_INTERLEAVE_ENABLE2
					| currentVal;
			break;
		case 14:
			adcInterleaveVal = (uint32_t) PIN_ADC_INTERLEAVE_ENABLE3
					| currentVal;
			break;
		case 15:
			adcInterleaveVal = (uint32_t) PIN_ADC_INTERLEAVE_DISABLE0
					& currentVal;
			break;
		case 16:
			adcInterleaveVal = (uint32_t) PIN_ADC_INTERLEAVE_DISABLE1
					& currentVal;
			break;
		default:
			/* invalid command */
		...
		switch (pinPortIdx) {
		case 0:
			adcInterleaveVal = (uint32_t) PIN_ADC_INTERLEAVE_DISABLE2
					& currentVal;
			break;
		case 1:
			adcInterleaveVal = (uint32_t) PIN_ADC_INTERLEAVE_DISABLE3
					& currentVal;
			break;
		default:
			/* invalid command */
...
void PINS_Init(const pin_settings_config_t *config) {
	...
	if (muxing == PORT_MUX_ADC_INTERLEAVE) {
		...
		uint32_t interleaveVal = PINS_GetAdcInterleaveVal(config->base,
				config->pinPortIdx, chipCtlReg);
	...
	digitalFilters |= (((uint32_t) (config->digitalFilter))
			<< (config->pinPortIdx));
	...
	if (PORT_MUX_AS_GPIO == config->mux) {
		...
		if (config->direction == GPIO_OUTPUT_DIRECTION) {
			PINS_GPIO_WritePin(config->gpioBase, config->pinPortIdx,
					config->initValue);
		}
 ...
 *
 *END**************************************************************************/
void PINS_SetMuxModeSel(PORT_Type *const base, uint32_t pin, port_mux_t mux) {
	...
	if (muxing == PORT_MUX_ADC_INTERLEAVE) {
		...
		uint32_t chipCtlReg =
				(SIM->CHIPCTL & SIM_CHIPCTL_ADC_INTERLEAVE_EN_MASK)
						>> SIM_CHIPCTL_ADC_INTERLEAVE_EN_SHIFT;
		uint32_t interleaveVal = PINS_GetAdcInterleaveVal(base, pin,
				chipCtlReg);
		if (interleaveVal != (uint32_t) PIN_ADC_INTERLEAVE_INVALID) {
			SIM->CHIPCTL &= ~(SIM_CHIPCTL_ADC_INTERLEAVE_EN_MASK);
			SIM->CHIPCTL |= SIM_CHIPCTL_ADC_INTERLEAVE_EN(interleaveVal);
 ...
 *
 *END**************************************************************************/
void PINS_SetGlobalPinControl(PORT_Type *const base, uint16_t pins,
		uint16_t value, port_global_control_pins_t halfPort) {
	uint16_t mask = 0;
	...
	case PORT_GLOBAL_CONTROL_LOWER_HALF_PINS:
		base->GPCLR = (((uint32_t) pins) << PORT_GPCLR_GPWE_SHIFT)
				| (uint32_t) mask;
		break;
	case PORT_GLOBAL_CONTROL_UPPER_HALF_PINS:
		base->GPCHR = (((uint32_t) pins) << PORT_GPCLR_GPWE_SHIFT)
				| (uint32_t) mask;
		break;
	default:
		/* nothing to configure */
 ...
 *
 *END**************************************************************************/
void PINS_SetGlobalIntControl(PORT_Type *const base, uint16_t pins,
		uint16_t value, port_global_control_pins_t halfPort) {
	uint32_t mask;
