...
status_t EDMA_DRV_Init(edma_state_t *edmaState,
		...
		const edma_channel_config_t *const chnConfigArray[], uint32_t chnCount) {
	uint32_t index = 0U;
	...
	for (index = 0U;
			index < (uint32_t) FEATURE_DMA_VIRTUAL_ERROR_INTERRUPT_LINES;
			index++) {
	...
	for (index = 0U;
			index < (uint32_t) FEATURE_DMA_VIRTUAL_CHANNELS_INTERRUPT_LINES;
			index++) {
		/* Enable channel interrupt ID. */
...
status_t EDMA_DRV_Deinit(void) {
	...
	if (s_virtEdmaState != NULL) {
		...
		for (index = 0U; index < (uint32_t) FEATURE_DMA_VIRTUAL_CHANNELS;
				index++) {
...
status_t EDMA_DRV_ChannelInit(edma_chn_state_t *edmaChannelState,
	...
	DEV_ASSERT(
			s_virtEdmaState->virtChnState[edmaChannelConfig->virtChnConfig] == NULL);
	...
	EDMA_DRV_ClearStructure((uint8_t*) edmaChannelState,
			sizeof(edma_chn_state_t));
...
status_t EDMA_DRV_InstallCallback(uint8_t virtualChannel,
		edma_callback_t callback, void *parameter) {
...
status_t EDMA_DRV_ConfigSingleBlockTransfer(uint8_t virtualChannel,
		edma_transfer_type_t type, uint32_t srcAddr, uint32_t destAddr,
	...
	if (retStatus == STATUS_SUCCESS) {
		...
		case EDMA_TRANSFER_PERIPH2MEM:
			...
			EDMA_TCDSetDestOffset(edmaRegBase, dmaChannel,
					(int8_t) transferOffset);
			break;
		case EDMA_TRANSFER_MEM2PERIPH:
			EDMA_TCDSetSrcOffset(edmaRegBase, dmaChannel,
					(int8_t) transferOffset);
...
status_t EDMA_DRV_ConfigMultiBlockTransfer(uint8_t virtualChannel,
	...
	retStatus = EDMA_DRV_ConfigSingleBlockTransfer(virtualChannel, type,
			srcAddr, destAddr, transferSize, blockSize);
...
status_t EDMA_DRV_ConfigLoopTransfer(uint8_t virtualChannel,
	...
	DEV_ASSERT(
			(transferConfig->srcModulo == EDMA_MODULO_OFF) || ((transferConfig->srcAddr % (((uint32_t)1U) << (uint32_t)transferConfig->srcModulo)) == 0U));
	DEV_ASSERT(
			(transferConfig->destModulo == EDMA_MODULO_OFF) || ((transferConfig->destAddr % (((uint32_t)1U) << (uint32_t)transferConfig->destModulo)) == 0U));
...
status_t EDMA_DRV_ConfigScatterGatherTransfer(uint8_t virtualChannel,
		edma_software_tcd_t *stcd, edma_transfer_size_t transferSize,
		uint32_t bytesOnEachRequest, const edma_scatter_gather_list_t *srcList,
	...
	EDMA_DRV_ClearStructure((uint8_t*) (&edmaTransferConfig),
			sizeof(edma_transfer_config_t));
	EDMA_DRV_ClearStructure((uint8_t*) (&edmaLoopConfig),
			sizeof(edma_loop_transfer_config_t));
	...
	for (i = 0U; (i < tcdCount) && (retStatus == STATUS_SUCCESS); i++) {
		...
		if ((srcList[i].length != destList[i].length)
				|| (srcList[i].type != destList[i].type)) {
			retStatus = STATUS_ERROR;
			break;
		}
		edmaTransferConfig.loopTransferConfig->majorLoopIterationCount =
				srcList[i].length / bytesOnEachRequest;
		...
		if (i == ((uint8_t) (tcdCount - 1U))) {
			...
			edmaTransferConfig.scatterGatherNextDescAddr =
					((uint32_t) ptNextAddr);
		}
		...
		if (i == 0U) {
			...
			EDMA_DRV_PushConfigToSTCD(&edmaTransferConfig,
					&edmaSwTcdAddr[i - 1U]);
		}
	}
...
status_t EDMA_DRV_SetChannelRequestAndTrigger(uint8_t virtualChannel,
	...
	uint8_t dmaMuxInstance = (uint8_t) FEATURE_DMAMUX_REQ_SRC_TO_INSTANCE(
			request);
	...
	uint8_t dmaMuxChannel = (uint8_t) FEATURE_DMAMUX_DMA_CH_TO_CH(
			virtualChannel);
...
void EDMA_DRV_CancelTransfer(bool error) {
	...
	for (dmaInstance = 0U; dmaInstance < (uint32_t) DMA_INSTANCE_COUNT;
			dmaInstance++) {
...
void EDMA_DRV_PushConfigToSTCD(const edma_transfer_config_t *config,
	...
	if ((config != NULL) && (stcd != NULL)) {
		...
		stcd->CITER =
				(uint16_t) config->loopTransferConfig->majorLoopIterationCount;
		...
		stcd->CSR = (uint16_t) (((config->interruptEnable ? 1UL : 0UL)
				...
				| ((config->scatterGatherEnable ? 1UL : 0UL)
...
void EDMA_DRV_PushConfigToReg(uint8_t virtualChannel,
	...
	EDMA_TCDSetAttribute(edmaRegBase, dmaChannel, tcd->srcModulo,
			tcd->destModulo, tcd->srcTransferSize, tcd->destTransferSize);
	...
	if (tcd->scatterGatherEnable) {
		...
		EDMA_TCDSetDestLastAdjust(edmaRegBase, dmaChannel,
				tcd->destLastAddrAdjust);
	}
	...
	if (tcd->loopTransferConfig != NULL) {
		...
		EDMA_TCDSetMinorLoopOffset(edmaRegBase, dmaChannel,
				tcd->loopTransferConfig->minorLoopOffset);
		EDMA_TCDSetNbytes(edmaRegBase, dmaChannel, tcd->minorByteTransferCount);

		EDMA_TCDSetChannelMinorLink(edmaRegBase, dmaChannel,
				tcd->loopTransferConfig->minorLoopChnLinkNumber,
				tcd->loopTransferConfig->minorLoopChnLinkEnable);
		EDMA_TCDSetChannelMajorLink(edmaRegBase, dmaChannel,
				tcd->loopTransferConfig->majorLoopChnLinkNumber,
				tcd->loopTransferConfig->majorLoopChnLinkEnable);

		EDMA_TCDSetMajorCount(edmaRegBase, dmaChannel,
				tcd->loopTransferConfig->majorLoopIterationCount);
	} else {
		EDMA_TCDSetNbytes(edmaRegBase, dmaChannel, tcd->minorByteTransferCount);
	}
}
